...
export default function EditTransactionScreen({ route, navigation }: any) {
    ...
    const handleSave = async () => {
        ...
        if (!description.trim()) {
            crossAlert('Erro', 'Por favor, insira uma descrição');
            return;
        }
        ...
        try {
            ...
            if (transaction.type === 'expense' && dueDate) {
                ...
                if (isNaN(day) || day < 1 || day > 31 || dueDate.length !== 2) {
                    setIsSaving(false);
                    crossAlert('Erro', 'Dia de vencimento deve ter 2 dígitos (Ex: 01, 15, 30)');
                    return;
                }
            ...
            if (transaction.type === 'income' && receivedDate) {
                ...
                if (isNaN(day) || day < 1 || day > 31 || receivedDate.length !== 2) {
                    setIsSaving(false);
                    crossAlert('Erro', 'Dia de recebimento deve ter 2 dígitos (Ex: 01, 15, 28)');
                    return;
                }
            ...
            if (transaction.isSalary && transaction.id.startsWith('salary_')) {
                ...
                await FirestoreService.updateTransaction(transaction.id, {
                    ...
                    ...(transaction.type === 'expense' && { dueDate: dueDateISO || deleteField() }),
                    ...(transaction.type === 'expense'
                        ? selectedCardId
                            ? { cardId: selectedCardId, cardName: selectedCardName, cardType: selectedCardType }
                            : { cardId: deleteField(), cardName: deleteField(), cardType: deleteField() }
                        : { cardId: deleteField(), cardName: deleteField(), cardType: deleteField() }
                    ),
                    ...(transaction.type === 'income' && { receivedDate: receivedDateISO || deleteField() }),
                });
            }

            crossAlert('Sucesso', 'Transação atualizada com sucesso!', [
                { text: 'OK', onPress: () => navigation.goBack() },
            ]);
        } catch (error) {
            console.error('Error updating transaction:', error);
            crossAlert('Erro', 'Não foi possível atualizar a transação');
        } finally {
            setIsSaving(false);
        }
    };
                    ...
                    { text: 'Cancelar', style: 'cancel' },
                    {
                        ...
                        onPress: async () => {
                            ...
                            try {
                                ...
                                crossAlert('Sucesso', 'Excluído com sucesso!', [
                                    { text: 'OK', onPress: () => navigation.goBack() },
                                ]);
                            } catch (error) {
                    ...
                    { text: 'Cancelar', style: 'cancel' },
                    {
                        ...
                        onPress: async () => {
                            ...
                            try {
                                ...
                                crossAlert('Sucesso', 'Transação excluída!', [
                                    { text: 'OK', onPress: () => navigation.goBack() },
                                ]);
                            } catch (error) {
                                console.error('Error deleting:', error);
    ...
    const handleCancelRecurrence = () => {
        ...
        if (!transaction.recurrenceId) {
            crossAlert('Aviso', 'Esta transação não possui recorrência vinculada.');
            return;
        }
                ...
                { text: 'Não', style: 'cancel' },
                {
                    ...
                    onPress: async () => {
                        ...
                        try {
                            ...
                            crossAlert('Erro', 'Não foi possível cancelar a recorrência');
                        } finally {
                            setIsSaving(false);
                        }
                    },
            ...
            <ScrollView contentContainerStyle={styles.content}>
                ...
                <View style={styles.actionsRow}>
                    {transaction.isRecurring && transaction.recurrenceId && (
                        <TouchableOpacity
                            style={[styles.cancelRecurrenceButton, isSaving && { opacity: 0.5 }]}
