...
export default function CardDetailsScreen({ route, navigation }: any) {
    ...
    const loadCardDetails = async () => {
        try {
            ...
            if (!foundCard) {
                crossAlert('Erro', 'Cartão não encontrado');
                navigation.goBack();
                return;
            }
            setCard(foundCard);
            ...
            crossAlert('Erro', 'Não foi possível carregar os detalhes do cartão');
        }
    };
    ...
    const togglePaidStatus = async (period: string) => {
        ...
        try {
            ...
            crossAlert('Erro', 'Não foi possível atualizar o status de pagamento');
        }
    };
